...
export default async function postgresClient(credentials: Credentials): Promise<Client> {
  ...
  const databaseError = `Postgress: The database called ${credentials.database} does not exist on the current Postgress server`;
  const authError = `Postgress: The user ${credentials.user} or its password do not match the records in the database called ${credentials.database}`;
